...
export const meta: Route.MetaFunction = ({ params }) => {
  // URL 파라미터에서 year와 month를 가져와서 월 날짜 객체 생성
  ...
  // 페이지 제목 반환: "The best of [연도년 월월] | wemake" 형식
  return [
    {"title": `The best of ${date.toFormat('yyyy년 MM월')} | wemake` }
  ]
}

// URL 파라미터로 전달된 year, month를 숫자로 강제 변환하고 유효성을 검사하기 위한 스키마 정의
...
export const loader = ({ params }: Route.LoaderArgs) => {
  ...
  if (!success) {
    ...
    throw data(
      { 
        ...
        message: "유효하지 않은 월 파라미터입니다. year, month는 숫자여야 합니다.",
        details: "URL에 올바른 월 형식(예: /2025/12)을 입력해주세요."
      }, 
      { status: 400 }
    );
  }
  
  // Luxon의 DateTime.fromObject를 사용해 parsedData 객체를 월 날짜 객체로 변환
  // Asia/Seoul 타임존으로 설정
  const date = DateTime.fromObject({
  ...
  // 변환된 월 날짜 객체가 유효하지 않은 경우 (예: 2025/13 같은 월)
  if (!date.isValid) {
    throw data(
      {
        error_code: "INVALID_MONTH_VALUE",
        message: "존재하지 않는 월입니다.",
        details: `입력된 월 ${parsedData.year}-${parsedData.month}는 유효하지 않습니다.`,
        suggested_date: "올바른 월 형식(예: 2025-12)을 입력해주세요."
      }, 
      { status: 400 }
    );
  } 
  
  // 현재 월을 Asia/Seoul 타임존 기준으로 가져오고, 월의 시작으로 초기화 (startOf('month'))
  const currentMonth = DateTime.now().setZone("Asia/Seoul").startOf("month");
  
  // 입력된 월이 현재 월보다 미래인 경우 에러 처리
  if (date > currentMonth) {
    throw data(
      {
        error_code: "FUTURE_MONTH_NOT_ALLOWED",
        message: "미래 월은 허용되지 않습니다.",
        details: `입력된 월 ${date.toFormat('yyyy년 MM월')}는 현재 월(${currentMonth.toFormat('yyyy년 MM월')}) 이후입니다.`,
        suggested_date: "현재 월 또는 과거 월을 선택해주세요."
      },
      { status: 400 }
    )
  }

  // loader 함수의 반환값: 유효한 월 정보를 포함하는 객체 반환
  return {
    ...parsedData
  }
}
...
export default function MonthlyLeaderboardPage({ loaderData }: Route.ComponentProps) {
  ...
  // loaderData에서 year와 month를 가져와서 월 날짜 객체 생성
  ...
  // 현재 월인지 확인
  const isCurrentMonth = urlDate.equals(DateTime.now().startOf("month"));
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Hero section */}
      <HeroSection 
        title={`The best of ${urlDate.toFormat('yyyy년 MM월')}`}
      />
      ...
      <div className="flex items-center justify-center gap-2">
        <Button variant="secondary" asChild>
          <Link to={`/products/leaderboards/monthly/${previousMonth.year}/${previousMonth.month}`}>
            &larr; {previousMonth.toFormat('yyyy년 MM월')}
          </Link>
        </Button>
        {
          !isCurrentMonth ? (
          <Button variant="secondary" asChild>
            ...
            {nextMonth.toFormat('yyyy년 MM월')} &rarr;
          </Link>
          </Button>
          ) : null
        }
