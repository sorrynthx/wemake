...
export const meta: Route.MetaFunction = ({ params }) => {
  // URL 파라미터에서 year와 week를 가져와서 주차 날짜 객체 생성
  ...
  // 페이지 제목 반환: "The best of [연도년 주차주차] | wemake" 형식
  return [
    {"title": `The best of ${date.toFormat('yyyy년 W주차')} | wemake` }
  ]
}

// URL 파라미터로 전달된 year, week를 숫자로 강제 변환하고 유효성을 검사하기 위한 스키마 정의
const paramsSchema = z.object({
  year: z.coerce.number(),  // year를 숫자로 변환
  week: z.coerce.number()   // week를 숫자로 변환
});
...
export const loader = ({ params }: Route.LoaderArgs) => {
  ...
  if (!success) {
    ...
    throw data(
      { 
        ...
        message: "유효하지 않은 주차 파라미터입니다. year, week는 숫자여야 합니다.",
        details: "URL에 올바른 주차 형식(예: /2025/33)을 입력해주세요."
      }, 
      { status: 400 }
    );
  }
  
  // Luxon의 DateTime.fromObject를 사용해 parsedData 객체를 주차 날짜 객체로 변환
  // Asia/Seoul 타임존으로 설정
  const date = DateTime.fromObject({
  ...
  // 변환된 주차 날짜 객체가 유효하지 않은 경우 (예: 2025/53 같은 주차)
  if (!date.isValid) {
    throw data(
      {
        error_code: "INVALID_WEEK_VALUE",
        message: "존재하지 않는 주차입니다.",
        details: `입력된 주차 ${parsedData.year}-W${parsedData.week}는 유효하지 않습니다.`,
        suggested_date: "올바른 주차 형식(예: 2025-W33)을 입력해주세요."
      }, 
      { status: 400 }
    );
  } 
  
  // 현재 주차를 Asia/Seoul 타임존 기준으로 가져오고, 주의 시작으로 초기화 (startOf('week'))
  const currentWeek = DateTime.now().setZone("Asia/Seoul").startOf("week");
  
  // 입력된 주차가 현재 주차보다 미래인 경우 에러 처리
  if (date > currentWeek) {
    throw data(
      {
        error_code: "FUTURE_WEEK_NOT_ALLOWED",
        message: "미래 주차는 허용되지 않습니다.",
        details: `입력된 주차 ${date.toFormat('yyyy년 W주차')}는 현재 주차(${currentWeek.toFormat('yyyy년 W주차')}) 이후입니다.`,
        suggested_date: "현재 주차 또는 과거 주차를 선택해주세요."
      },
      { status: 400 }
    )
  }

  // loader 함수의 반환값: 유효한 주차 정보를 포함하는 객체 반환
  return {
    ...parsedData
  }
}
...
export default function WeeklyLeaderboardPage({ loaderData }: Route.ComponentProps) {
  ...
  // loaderData에서 year와 week를 가져와서 주차 날짜 객체 생성
  ...
  // 현재 주차인지 확인
  const isCurrentWeek = urlDate.equals(DateTime.now().startOf("week"));
  ...
  return (
    <div className="space-y-10">
      ...
      <HeroSection 
        title={`The best of  
          ${urlDate.startOf("week").toLocaleString(DateTime.DATE_SHORT)} 
          - 
      ...
      <div className="flex items-center justify-center gap-2">
        <Button variant="secondary" asChild>
          <Link to={`/products/leaderboards/weekly/${previousWeek.weekYear}/${previousWeek.weekNumber}`}>
            &larr; {previousWeek.startOf("week").toLocaleString(DateTime.DATE_SHORT)}
          </Link>
        </Button>
        {
          !isCurrentWeek ? (
          <Button variant="secondary" asChild>
            ...
            {nextWeek.startOf("week").toLocaleString(DateTime.DATE_SHORT)} &rarr;
          </Link>
          </Button>
          ) : null
        }
